...
@Data
@Schema(description = "更新比赛请求")
public class UpdateContestRequest {
    @Schema(description = "待更新的比赛 ID", example = "2001", requiredMode = Schema.RequiredMode.REQUIRED)
    private Long contestId;

    @Schema(description = "比赛名称，去首尾空格后不能为空，长度不超过 50", example = "TenJudge April Challenge Finals", requiredMode = Schema.RequiredMode.REQUIRED)
    private String name;
    ...
    @ArraySchema(
        arraySchema = @Schema(description = "比赛题目编排列表。更新时采用全量覆盖策略；传空列表表示清空当前编排"),
        schema = @Schema(implementation = ContestProblemDTO.class)
    )
